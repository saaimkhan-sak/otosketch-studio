...
function isWithinLandmark(
  actual: MedicalArtSourcePoint,
  landmark: FixtureLandmark,
) {
...
function countAlphaOverlap(
  ...
  const lastX = Math.min(
    left.offset.x + left.width,
    right.offset.x + right.width,
  );
  const lastY = Math.min(
    left.offset.y + left.height,
    right.offset.y + right.height,
  );
  let count = 0;
  ...
  for (let sourceY = firstY; sourceY < lastY; sourceY += 1) {
    for (let sourceX = firstX; sourceX < lastX; sourceX += 1) {
      const leftIndex =
        ((sourceY - left.offset.y) * left.width +
          (sourceX - left.offset.x)) *
          left.channels +
        3;
      const rightIndex =
        ((sourceY - right.offset.y) * right.width +
          (sourceX - right.offset.x)) *
          right.channels +
        3;
      if (
        left.data[leftIndex] >= threshold &&
        right.data[rightIndex] >= threshold
      ) {
        count += 1;
      }
    }
...
function alphaAtSourcePoint(
  ...
  return image.data[
    (localY * image.width + localX) * image.channels + 3
  ];
}

async function rasterizedPathAlpha(
  pathData: string,
  width = 584,
  height = 370,
) {
...
describe("Servier native-source placement gold standard", () => {
  it("binds the calibration to the licensed source canvas and exact official files", async () => {
    ...
    expect(
      sha256(await readFile(sourcePath(sourceAsset.localPath))),
    ).toBe(fixture.source.compositeSha256);

    expect(sourceComponents).toHaveLength(
      Object.keys(fixture.components).length,
    );
    for (const [componentId, expected] of Object.entries(
      ...
      [
        FixtureComponentId,
        (typeof fixture.components)[FixtureComponentId],
      ]
    >) {
      const component = sourceComponents.find(
        (candidate) => candidate.id === componentId,
  ...
  it("keeps PORP and TORP contacts inside the independently audited landmark tolerances", () => {
    ...
    expect(
      isWithinLandmark(
        porp.tympanicSurfaceContact,
        fixture.landmarks.tympanicSurfaceContact,
      ),
    ).toBe(true);
    expect(
      isWithinLandmark(
        torp.tympanicSurfaceContact,
        fixture.landmarks.tympanicSurfaceContact,
      ),
    ).toBe(true);
    expect(
      isWithinLandmark(porp.distalContact.center, fixture.landmarks.capitulum),
    ).toBe(true);
    expect(
      isWithinLandmark(torp.distalContact.center, fixture.landmarks.footplate),
    ).toBe(true);
  ...
  it("derives the tympanic stack from the audited plane and normal offsets", () => {
    const geometry =
      medicalArtSourceGeometry["servier-inner-ear"]?.tympanicMembrane;
  ...
  it("keeps the entire headplate inside its protective cartilage layer", () => {
    ...
    for (const method of ["porp", "torp"] as const) {
      ...
      for (let sample = 0; sample < 720; sample += 1) {
        const point = ellipseBoundaryPoint(
          placement.headPlate,
  ...
  it("uses neutral generic contacts without inventing a manufacturer-specific cup or shoe", () => {
    ...
    expect(porp.rendering).toBe(
      "neutral-headplate-shaft-capitulum-seat",
    );
    expect(torp.rendering).toBe(
  ...
  it("keeps central, posterior, and subtotal tympanic regions geometrically distinct", () => {
    ...
    expect(distance(centralCentroid, posteriorCentroid)).toBeGreaterThan(10);
    expect(sourcePolygonArea(subtotal)).toBeGreaterThan(
      sourcePolygonArea(central) * 4,
    );
  });
  ...
  it("maps every cutaway perforation and covering graft inside the audited tympanic surface", async () => {
    ...
    const tympanicMembrane =
      medicalArtSourceGeometry["servier-ear-cutaway"]?.tympanicMembrane;
    ...
    expect(
      sha256(await readFile(sourcePath(asset.localPath))),
    ).toBe(cutawayFixture.source.sha256);
    ...
    expect(
      alphaBounds(
        support,
        cutawayFixture.tympanicSupport.alphaThreshold,
      ),
    ).toEqual({
    ...
    for (const [region, expected] of Object.entries(
      ...
      const perforation = deriveMedicalArtTympanicPolygon(
        "servier-ear-cutaway",
        region,
        template,
      );
      ...
      expect(
        sourcePolygonArea(perforation),
        `${region} perforation area`,
      ).toBeCloseTo(expected.area, 0);
      expect(
        sourcePolygonArea(graft),
        `${region} graft area`,
      ).toBeCloseTo(expected.graftArea, 0);
      expect(
        sourcePolygonArea(graft) / sourcePolygonArea(perforation),
        `${region} graft coverage ratio`,
      ).toBeGreaterThanOrEqual(
        cutawayFixture.tolerances.minimumGraftAreaRatio,
      );
    ...
    const anterior = deriveMedicalArtTympanicPolygon(
      "servier-ear-cutaway",
      "anterior",
    );
    const posterior = deriveMedicalArtTympanicPolygon(
      "servier-ear-cutaway",
      "posterior",
    );
    ...
    expect(sourcePolygonCentroid(posterior).y).toBeGreaterThan(
      sourcePolygonCentroid(anterior).y,
    );
  });
